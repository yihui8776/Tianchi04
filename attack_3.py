...
parser.add_argument('--save', default='final', help='folder to output images and model checkpoints')
...
def get_mask3(image, meta, pixels):
    ...
    #tarea = 0
    #for i in range(num):
        #tarea += (int(bbox[i,2])-int(bbox[i,0]))+(int(bbox[i,3])-int(bbox[i,1]))     
...
def get_mask4(image, meta, pixels):
    ...
    for i in range(num):
        ...
        if label[i] !=0:
            xc = int((bbox[i,0]+bbox[i,2])/4)
            yc = int((bbox[i,1]+bbox[i,3])/4) 
...
for file in files:
    ...
    mask1 = get_mask3(data['img'], data['img_metas'], pixels)
    mask2 = get_mask4(data['img'], data['img_metas'], pixels)
    ...
    if flag == 0:
        ...
        optimizer = optim.SGD([patch], lr=32/255.)
        #optimizer = optim.SGD([patch], lr=256/255.)
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer,  milestones = [10, 25, 45, 70, 110], gamma = 0.5, last_epoch=-1)        
        if record1 > record2:
