...
parser.add_argument('--max_iter', type=int, default=300, help='max number of iterations to find adversarial example')
...
parser.add_argument('--save', default='select1000_new_p', help='folder to output images and model checkpoints')
...
def get_mask(image, meta, pixels):
    ...
    for i in range(len(m[0])):
        if len(m[0][i]) > 0:  #取出非空的元素 就是有检测出的label ，id对应数组下标
            print(len(m[0][i]))
            for j in range(len(m[0][i])):  #第i个label的包围盒个数
...
for file in files:
    ...
    if flag == 0:
        ...
        patch.requires_grad = True        
        optimizer = optim.SGD([patch], lr=32/255.)
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer,  milestones = [10, 25, 50, 80, 115], gamma = 0.5, last_epoch=-1) 
        for i in range(150):
            scheduler.step()
            imgp = torch.mul(img, 1-mask) + torch.mul(patch, mask)
            Img1 = F.interpolate(imgp, size=(608, 608), mode='bilinear', align_corners=True)
