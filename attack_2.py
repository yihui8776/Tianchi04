...
parser.add_argument('--save', default='final2', help='folder to output images and model checkpoints')
...
def get_mask(image, meta, pixels):
    ...
    for i in range(num):
        ...
        y1 = max(0, yc-lw//2)
        y2 = min(yc+lw//2, 500)
        mask[:,:,yc-1:yc+1,x1:x2] = 1
        mask[:,:,y1:y2,xc-1:xc+1] = 1
...
for file in files:
    ...
    if flag == 0:
        ...
        optimizer = optim.SGD([patch], lr=32/255.)
        #optimizer = optim.SGD([patch], lr=256/255.)
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer,  milestones = [10, 25, 45, 75, 110], gamma = 0.5, last_epoch=-1)        
        if record1 > record2:
