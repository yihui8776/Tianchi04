...
parser.add_argument('--save', default='select1000_new_p2', help='folder to output images and model checkpoints')
...
def get_mask(image, meta, pixels):
    ...
    for i in range(num):
        ...
        x1 = max(0, xc-lh//4)
        x2 = min(xc+lh//4, 500)        
        mask[:,:,y1:y1+3,x1:x2] = 1
        mask[:,:,y2-2:y2+1,x1:x2] = 1
...
def get_mask2(image, meta, pixels):
    ...
    for i in range(num):
        ...
        mask[:,:,y1:y2,x1:x1+3] = 1
        mask[:,:,y1:y2,x2-2:x2+1] = 1
    mask = mask.float().cuda()    
